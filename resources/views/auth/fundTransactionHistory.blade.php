...
<body>
    ...
    <div class="content" id="content">
        <h4> Fund Transaction History </h4>
        <br>
...
<script>
    document.getElementById('fund-trans-history-form').addEventListener('submit', function(event) {
        ...
        const url = `/api/fund-transaction-history/${encodeURIComponent(accountNumber)}`;
        ...
        .then(response => response.json().then(data => {
            if (response.ok) {
                showSuccessAlert('Fund Transaction History Fetched Successfully!', 'success');
                ...
                console.error('Failed to Fetch Transaction History with Status:', response.status);
                showFailureAlert(
                    data.errors
                    ? Object.values(data.errors).flat().join(' ')
                    : 'Failed to Fetch Transaction History or, Account Not Mathced. Please try again.',
                    'error'
                );
            }
        }))
