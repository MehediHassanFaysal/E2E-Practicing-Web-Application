...
<head>
    ...
    <title>Fund Transfer | {{ $loginUser->name }} </title>
    <style>
        body {
            margin: 0;
...
<body>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <ul class="submenu">
            ...
            <li><a href="#fundTransferHistory">Fund Transactions</a></li>
        </ul>
    </div>
    ...
    <h4> Fund Transfer (Savings To Savings) </h4>
    <br>
    <form id="fund-transfer-form" action="{{ route('fund-transfer.store')}}" method="POST">
        @csrf
            <div class="form-group">
                <label for="sender_account_number">Your Account Number:</label>
                <input type="text" id="sender_account_number" name="sender_account_number" value="{{ $userInfo->account_number ?? '' }}" readonly>
            </div>
            <div class="form-group">
                <label for="receiver_account_number">Receiver Account Number:</label>
                <input type="text" id="receiver_account_number" name="receiver_account_number" placeholder="Account Number">
            </div>
...
<script>
document.getElementById('fund-transfer-form').addEventListener('submit', function(event) {
    ...
    .then(response => response.json().then(data => {
        if (response.ok) {
            showSuccessAlert('Fund Transfer Successful!', 'success');

            // Clear the form fields
            form.reset();

            // Redirect to the landing page with session ID
            const baseUrl = '/fund-transfer';
    ...
    .catch(error => {
        ...
        showFailureAlert('Transaction Failed. Please try again.', 'error');
    });
});
</script>
