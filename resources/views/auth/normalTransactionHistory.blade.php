...
<head>
    ...
    <title>Cash Deposit Transaction History | {{ $loginUser->name }} </title>
    <style>
        body {
            margin: 0;
...
<body>
    ...
    <div class="content" id="content">
        <h4> Cash Deposit Transaction History </h4>
        <br>
...
<script>
    document.getElementById('fund-trans-history-form').addEventListener('submit', function(event) {
        ...
        const url = `/api/simple-transaction-history/${encodeURIComponent(accountNumber)}`;
        ...
        .then(response => response.json().then(data => {
            if (response.ok) {
                showSuccessAlert('Deposit Transaction History Fetched Successfully!', 'success');
                ...
                console.error('Failed to Fetch Deposit Transaction History with Status:', response.status);
                showFailureAlert(
                    data.errors
                    ? Object.values(data.errors).flat().join(' ')
                    : 'Failed to Fetch Deposit Transaction History or, Account Not Mathced. Please try again.',
                    'error'
                );
            }
        }))
