...
<head>
    ...
    <title>Cash Withdrawal Transaction | {{ $loginUser->name }} </title>
    <style>
        body {
            margin: 0;
...
<body>
    ...
    <div class="form-group">
        ...
        <input type="text" id="username" name="username" placeholder="Customer Name" value="{{ $loginUser->name }}" required readonly>
    </div> 
    ...
    <div class="form-group">
        ...
        <input type="tel" id="mobile_number" name="mobile_number" placeholder="Mobile Number" value="{{ $userInfo->mobile_number ?? '' }}" required readonly>
    </div>

    <!-- NID Card -->
    <div class="form-group">
        <label for="nid_card">NID Number:</label>
        <input type="text" id="nid_card" name="nid_card" placeholder="NID Card" value="{{ $userInfo->nation_id_card ?? '' }}" required readonly>
    </div>

    <!-- Amount -->
    <div class="form-group">
        <label for="available-amount">Available Amount:</label>
        <input type="number" id="available-amount" name="available-amount" step="0.01" placeholder="Amount" value="{{ $userInfo->amount ?? '' }}" required>
    </div>
</div>
...
<div class="password-container">
    ...
    <form id="cash-withdraw-form" action="{{ route('cash-withdraw.store') }}" method="POST">
        @csrf
        <!-- <div class="error-message" id="error-message"> -->
            ...
            <div class="form-group">
                ...
                <input type="text" id="transaction_type" name="transaction_type" value="w" style="display:none">
            </div>

            <!-- Remarks -->
            <div class="form-group">
                <label for="remarks">Remarks:</label>
                <textarea id="remarks" name="remarks" placeholder="Remarks">Cash Withdraw</textarea>
            </div>
...
<script>
document.getElementById('cash-withdraw-form').addEventListener('submit', function(event) {
    ...
    .then(response => response.json().then(data => {
        if (response.ok) {
            showSuccessAlert('Cash Withdraw Transaction successful!', 'success');

            // Clear the form fields
            form.reset();

            // Redirect to the landing page with session ID
            const baseUrl = '/cash-withdraw';
            ...
            showFailureAlert(
                ...
                : 'Cash Withdraw Transaction Failed. Please try again.',
                'error'
            );
            // // Display error message in the DOM
            // if (errorMessage) {
            //     errorMessage.textContent = data.errors ? Object.values(data.errors).flat().join(' ') : 'Error occurred';
            // }
        }
    }))
    .catch(error => {
        console.error('Error:', error);
        showFailureAlert('Cash Withdraw Transaction Failed. Please try again.', 'error');
    });
});
</script>
